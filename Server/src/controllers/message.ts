...
export const createMessage = async (req: Request, res: Response) => {
    try {
        ...
        const message = await messageService.createMessageService(senderId, receiverId, content);
...
export const getMessagesBetweenUsers = async (req: Request, res: Response) => {
    ...
    try {
        const plainMessages = await messageService.getMessagesBetweenUsersService(currentUserId, otherUserId);
...
export const getSendersForCurrentUser = async (req: Request, res: Response) => {
    ...
    try {
       ...
       const uniqueSenders = await  messageService.getSendersForCurrentUserService(currentUserId);
        return res.status(200).json({
