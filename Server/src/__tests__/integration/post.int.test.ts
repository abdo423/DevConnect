...
describe("POST /post/create", () => {
    ...
    it("should create post successfully", async () => {
        ...
        // Mock validation
        jest.spyOn(postModule, 'validatePost')
        ...
        (Post as jest.MockedClass<typeof Post>).mockImplementation(() => mockSavedPost as any);
        ...
        (User.findByIdAndUpdate as jest.Mock).mockResolvedValue({ ...mockUser, posts: [mockSavedPost._id] });
        ...
        expect(mockPopulateAuthor).toHaveBeenCalledWith("author_id", "username email avatar");
        expect(mockPopulateComments).toHaveBeenCalledWith("comments", "content createdAt");
    });
    ...
    it("should return 400 if validation fails - title too short", async () => {
        ...
        expect(res.status).toBe(400);
        expect(res.body.message).toEqual({ title: "Title must be at least 3 characters" });
    });
    ...
    it("should return 400 if validation fails - content too short", async () => {
        ...
        expect(res.status).toBe(400);
        expect(res.body.message).toEqual({ content: "Content must be at least 10 characters" });
    });
    ...
    it("should return 500 if user update fails", async () => {
        ...
        (User.findById as jest.Mock).mockResolvedValue(mockUser);
        (User.findByIdAndUpdate as jest.Mock).mockRejectedValue(new Error("User update failed"));
    ...
    it("should update user's posts array correctly", async () => {
        ...
        (User.findById as jest.Mock).mockResolvedValue(mockUser);
        (User.findByIdAndUpdate as jest.Mock).mockResolvedValue({ ...mockUser, posts: [mockSavedPost.id] });
    ...
    describe("DELETE /post/delete/:id", () => {
        ...
        it("should return 500 if delete operation fails", async () => {
            ...
            (Post.findById as jest.Mock).mockResolvedValue(mockPost);
            (Post.deleteOne as jest.Mock).mockRejectedValue(new Error("Delete failed"));
    ...
    describe("PATCH /post/update/:id", () => {
        ...
        it("should update post successfully", async () => {
            ...
            jest.spyOn(require('../../models/post'), 'validateUpdatePost')
