...
describe('Comment Service', () => {
  ...
  describe('createComment', () => {
    ...
    it('should create comment successfully', async () => {
      ...
      expect(mockComment.populate).toHaveBeenCalledWith(
        'user',
        'username avatar',
      );
  ...
  describe('deleteComment', () => {
    ...
    it('should throw 400 if comment ID is invalid', async () => {
      ...
      await expect(commentService.deleteComment('invalid-id')).rejects.toEqual({
        status: 400,
  ...
  describe('updateComment', () => {
    ...
    it('should throw 400 if content is missing', async () => {
      await expect(
        commentService.updateComment(validCommentId, {
          content: undefined,
        } as any),
      ).rejects.toEqual({
        status: 400,
  ...
  describe('likeComment', () => {
    const mockCommentWithLikes = {
      ...
      likes: [
        {
          user: new Types.ObjectId('64c9f5a7b9e7c4e8d1234569'),
          createdAt: new Date(),
        },
      ],
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle populate errors in createComment', async () => {
      ...
      mockComment.save = jest.fn().mockResolvedValue(mockComment);
      mockComment.populate = jest
        .fn()
        .mockRejectedValue(new Error('Populate failed'));
    ...
    it('should handle database errors in deleteComment', async () => {
      (Comment.findById as jest.Mock).mockResolvedValue(mockComment);
      (Comment.deleteOne as jest.Mock).mockRejectedValue(
