...
describe('Message Controller', () => {
  ...
  describe('createMessage', () => {
    it('should create a message and return status 201', async () => {
      ...
      jest
        .spyOn(messageService, 'createMessageService')
        .mockResolvedValue(
          mockMessage as unknown as Awaited<
            ReturnType<typeof messageService.createMessageService>
          >,
        );
  ...
  describe('getMessagesBetweenUsers', () => {
    it('should get messages between users and return status 200', async () => {
      ...
      jest
        .spyOn(messageService, 'getMessagesBetweenUsersService')
        .mockResolvedValue(
          mockMessages as unknown as Awaited<
            ReturnType<typeof messageService.getMessagesBetweenUsersService>
          >,
        );

      await messageController.getMessagesBetweenUsers(
        req as Request,
        res as Response,
      );

      expect(
        messageService.getMessagesBetweenUsersService,
      ).toHaveBeenCalledWith(userId, receiverId);
    ...
    it('should return empty messages array when no messages exist', async () => {
      ...
      jest
        .spyOn(messageService, 'getMessagesBetweenUsersService')
        .mockResolvedValue(emptyMessages);
  ...
  describe('getSendersForCurrentUser', () => {
    it('should return senders and count when successful', async () => {
      ...
      jest
        .spyOn(messageService, 'getSendersForCurrentUserService')
        .mockResolvedValue(
          mockSenders as unknown as Awaited<
            ReturnType<typeof messageService.getSendersForCurrentUserService>
          >,
        );

      await messageController.getSendersForCurrentUser(
        req as Request,
        res as Response,
      );
