...
#### Install Root Dependencies (for Husky hooks)
...
The project uses **Husky** for git hooks, **lint-staged** for running tasks on staged files, and **Prettier** for code formatting.

### Pre-commit Hooks

Git pre-commit hooks are automatically set up when you run `npm install` in the root directory. These hooks will:

- Automatically format staged files with Prettier before each commit
