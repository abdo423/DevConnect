...
vi.mock("@radix-ui/react-icons", () => ({
    CheckCircledIcon: ({ className }: { className?: string }) => (
        <div data-testid="success-icon" className={className}>Success Icon</div>
    ),
}))
...
describe("FormSuccess", () => {
    ...
    it("handles long success messages", () => {
        const longSuccessMessage = "This is a very long success message that should still be rendered correctly without breaking the component layout or functionality"
    ...
    it("has different styling from FormError component", () => {
        ...
        expect(successContainer).toHaveClass("bg-emerald-500/15", "text-emerald-500")
        expect(successContainer).not.toHaveClass("bg-destructive/35", "text-destructive")
    })
    ...
    describe("edge cases", () => {
        ...
        it("handles whitespace-only message", () => {
            ...
            expect(screen.getByTestId("form-success").querySelector("span")).toBeInTheDocument()
        })
