...
vi.mock('react-hook-form', () => ({
  useForm: vi.fn(() => ({
    handleSubmit: vi.fn(
      (fn: (data: MockFormData) => void) => (e: MockFormEvent) => {
        e.preventDefault();
...
vi.mock('@/components/ui/form', () => ({
  Form: ({ children }: GenericComponentProps) => (
  ...
  FormItem: ({ children }: GenericComponentProps) => (
...
describe('UpdatePostModal', () => {
  ...
  it('applies correct CSS classes and attributes', () => {
    ...
    const triggerButton = screen.getByText('Edit post');
    expect(triggerButton.closest('button')).toHaveAttribute(
      'data-variant',
      'ghost'
    );
    expect(triggerButton.closest('button')).toHaveAttribute(
