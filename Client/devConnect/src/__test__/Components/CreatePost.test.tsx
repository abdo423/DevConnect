...
vi.mock("@/features/Posts/postsSlice", async (importOriginal) => {
    const actual = await importOriginal<
        typeof import("@/features/Posts/postsSlice")
    >();
    return {
        ...actual,
...
describe("CreatePost", () => {
    ...
    it("shows button as disabled when submitting empty form", async () => {
        ...
        expect(screen.queryByText(/content must be at least 30 characters/i)).not.toBeInTheDocument();
    });
    ...
    it("dispatches createPostThunk when valid form is submitted", async () => {
        ...
        await act(async () => {
            fireEvent.change(contentTextarea, {
                target: { value: "This is valid content with more than 30 characters so it should pass validation" },
            });
        });
        ...
        await waitFor(() => {
            expect(createPostThunk).toHaveBeenCalledWith({
                title: "Valid Title",
                content: "This is valid content with more than 30 characters so it should pass validation",
                image: undefined,
            });
        });
    });
    ...
    it("shows character counter when typing", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByTestId("character-counter")).toHaveTextContent("12/280");
        });
    });
