...
import {
  fetchFollowings,
  fetchUnfollowedMessageSenders,
} from '@/features/Following/followingSlice.ts';
...
const MessagePage = () => {
  ...
  const { following, unfollowedMessageSenders } = useSelector(
  ...
  const filteredUnfollowedSenders = useMemo(() => {
    ...
    return (unfollowedMessageSenders || []).filter(
      (person: User) =>
        person.username?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        person.email?.toLowerCase().includes(searchQuery.toLowerCase())
    );
  ...
  const hasSearchResults =
    filteredFollowing.length > 0 || filteredUnfollowedSenders.length > 0;
            ...
            {selectedUser ? (
              <div className="space-y-4 max-w-4xl mx-auto">
                {messages.map((message) => (
                  ...
                  >
                    <div
                      onClick={() => profileNavigate(message.senderId)}
